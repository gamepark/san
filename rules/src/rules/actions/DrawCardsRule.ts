...
/**
 * Draw {@link Memory.RepeatCount} cards, one per pass, reshuffling the discard into the deck when it
...
export class DrawCardsRule extends SanRule {
  onRuleStart(): MaterialMove[] {
    const remaining = this.remind<number>(Memory.RepeatCount) ?? 0
    if (remaining <= 0) return this.done()

    if (this.deck.length === 0) {
      if (this.discard.length === 0) return this.done() // nothing left to draw
      return [...this.reshuffleDiscardIntoDeck(), this.startRule(RuleId.DrawCards)]
    }

    this.memorize(Memory.RepeatCount, remaining - 1)
