...
/** `move.data` is the item index of the copied played card (see CustomMoveType.CopyPlayedCard). */
export const CopyPlayedCardLog = ({ move, context }: MoveComponentProps<CustomMove>) => {
  const name = usePlayerName(context.action.playerId)
  const card = context.game.items[MaterialType.Card][move.data as number]

  return (
    <Trans
      i18nKey="history.copy.played"
      values={{ player: name }}
